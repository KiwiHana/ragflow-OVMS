...
class OpenAI_APIEmbed(OpenAIEmbed):
    ...
    def __init__(self, key, model_name, base_url):
        ...
        # Keep explicit API version paths (for example OVMS /v3) unchanged.
        # Only append /v1 for bare host/base URLs.
        if re.search(r"/v\d+$", normalized_base_url) or re.search(r"/openai/v\d+$", normalized_base_url):
            final_base_url = normalized_base_url
        else:
            final_base_url = urljoin(f"{normalized_base_url}/", "v1").rstrip("/")

        self.client = OpenAI(api_key=key, base_url=final_base_url)
        self.model_name = model_name.split("___")[0]
